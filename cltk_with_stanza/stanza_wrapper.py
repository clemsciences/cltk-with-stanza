...
from stanza.resources.prepare_resources import default_treebanks
...
class StanzaWrapper:
    ...
    def __init__(
        ...
        self.model_path = os.path.expanduser(
            os.path.join("~", "stanza_resources", self.stanza_code, "tokenize")+"/"
        )
        # if not self._is_model_present():
        # download model if necessary
        if not file_exists(self.model_path, is_dir=True):
    ...
    def _load_pipeline(self):
        ...
        elif self.language in ["chu", "got", "lzh"]:
            # Note: MWT not available for several languages
            processors = "tokenize,pos,lemma,depparse"
        elif self.language == "la":
            self.treebank = None
            processors = {"tokenize": "proiel", "pos": "proiel_nocharlm", "lemma": "proiel_nocharlm", "depparse": "proiel_nocharlm"}
